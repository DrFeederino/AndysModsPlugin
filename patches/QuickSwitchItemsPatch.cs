...
namespace AndysModsPlugin.patches
{
    ...
    internal static class QuickSwitchPatch
    {
        [HarmonyPatch("Start")]
        [HarmonyPostfix]
        internal static void AddQuickSwitchBehaviour(StartOfRound __instance)
        {
            ...
            AndysModsPlugin.Log.LogInfo($"QuickSwitch: registering QuickSwitchBehaviour to enable quick switching for player.");
            __instance.gameObject.AddComponent<QuickSwitchBehaviour>().Init();
        }
    }
    ...
    public class QuickSwitchInputClass : LcInputActions
    {
        ...
        public static readonly ModManager QuickSwitchMod = new();
        ...
        public void DisableExistingButtonActions()
        {
            if (playerInput == null)
            {
                playerInput = IngamePlayerSettings.Instance.playerInput;
            }
            AndysModsPlugin.Log.LogInfo("QuickSwitch: disabling all known InputActions to avoid collisions.");
        ...
        {
            AndysModsPlugin.Log.LogInfo("QuickSwitch: enabling mod's inputs.");
            Enable();
    ...
    internal class QuickSwitchBehaviour : MonoBehaviour
    {
        ...
        {
            QuickSwitchInputClass.Instance.DisableExistingButtonActions();
        ...
        {
            if (!QuickSwitchInputClass.QuickSwitchMod.IsEnabled)
            {
                QuickSwitchInputClass.Instance.DisableModInputActions();
                return;
            }
            else if (!QuickSwitchInputClass.Instance.Enabled)
            {
                QuickSwitchInputClass.Instance.EnableModInputActions();
            }
            if (player == null || StartOfRound.Instance?.localPlayerController != player)
            {
            ...
            if (player.currentItemSlot != keyNum)
            {
                ...
                if (((player.IsOwner && player.isPlayerControlled && (!player.IsServer || player.isHostPlayerObject)) || player.isTestingPlayer) && !((float)Traverse.Create(player).Field("timeSinceSwitchingSlots").GetValue() < 0.3f) && !player.isGrabbingObjectAnimation && !player.quickMenuManager.isMenuOpen && !player.inSpecialInteractAnimation && !(bool)Traverse.Create(player).Field("throwingObject").GetValue() && !player.isTypingChat && !player.twoHanded && !player.activatingItem && !player.jetpackControls && !player.disablingJetpackControls)
                {
                    ...
                    dynMethod.Invoke(player, [context]);
                }
            }
        }
